...
return(list(rH0= rejecth0 ,
  ...
  conjunctive.power  = if (sum(mu.exp != mu.null) == 0 ) print(" - ") else {if( sum(mu.exp != mu.null) > 1 ) mean( ifelse(rowSums(rejecth0[,which (mu.exp != mu.null)]) == length( which (mu.exp != mu.null) ), 1,0 ) ) else mean( rejecth0[,which (mu.exp != mu.null)]) },
                        ...
                        niterations)
{
  ...
  for(iteration in 1:niterations)
  {
    ...
    selectedarm=which.max(stage1.lr1[iteration, ])
